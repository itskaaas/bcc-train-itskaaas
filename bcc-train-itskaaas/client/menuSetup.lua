...
function MainMenu(station)
    ...
    if MyTrain then
        table.insert(elements, { label = _U('deleteTrain'), value = 'deleteTrain', desc = '' })
    end
    VORPMenu.Open('default', GetCurrentResourceName(), 'vorp_menu', {
        ...
        elseif data.current.value == 'deleteTrain' then
...
function DrivingMenu(trainCfg, myTrainData)
    ...
    if trainCfg.fuel.enabled then
        table.insert(elements, { label = _U('addFuel'), value = 'fuel', desc = '' })
    end
    if trainCfg.condition.enabled then
        table.insert(elements, { label = _U('repairTrain'), value = 'repair', desc = '' })
    end
    ...
    if trainCfg.fuel.enabled then
        table.insert(elements, { label = _U('fuel') .. ': ' .. tostring(TrainFuel), value = 'fuel_display', desc = '' })
    end
    if trainCfg.condition.enabled then
        table.insert(elements, { label = _U('condition') .. ': ' .. tostring(TrainCondition), value = 'condition_display', desc = '' })
    end
    ...
    function(data)
        ...
        local selectedOption = {
            ...
            ['fuel_display'] = function() end,
            ['condition_display'] = function() end,
        }
