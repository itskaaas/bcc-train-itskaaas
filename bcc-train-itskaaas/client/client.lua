...
local MiniGame = exports['itskaaas-lockpick'].initiate()
...
function SpawnTrain(trainCfg, myTrainData, dirChange, station) --credit to rsg_trains for some of the logic here
    ...
    TriggerServerEvent('bcc-train:UpdateTrainSpawnVar', true, netId, TrainId, GetEntityCoords(MyTrain))
...
CreateThread(function()
    while true do
        Wait(0)
        DisableControlAction(0, Config.keys.inventory, true)
...
AddEventHandler('playerDropped', function ()
    if MyTrain then
        DeleteEntity(MyTrain)
        MyTrain = nil
        TriggerServerEvent('bcc-train:UpdateTrainSpawnVar', false, nil, TrainId)
    end
end)

RegisterCommand('traininv', function()
    TriggerServerEvent('bcc-train:OpenInventory')
end)

RegisterNetEvent('bcc-train:StartLockpick')
AddEventHandler('bcc-train:StartLockpick', function(trainId)
    VORPcore.NotifyRightTip('Você pega a lockpick do bolso!', 4000)
    -- Minigame Config
    local cfg = {
    ...
    MiniGame.Start('lockpick', cfg, function(result)
        ...
        if success then
            ...
            VORPcore.NotifyRightTip(_U('PickAttemptFailed'), 4000)
